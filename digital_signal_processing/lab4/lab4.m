...
Q=0.1;%шум
%kp=4%количество периодов сигнала
 
%1. ГЕНЕРАЦИЯ МОДЕЛЬНОГО СИГНАЛА

% for kp = [3.0, 3.2, 3.4, 3.6, 3.8, 4.0]
for kp = [3.2]
    disp(['Q: ', num2str(Q)]);
    disp(['kp: ', num2str(kp)]);
    ...
    kp_bpf=kpbpf;
    disp(['kp_bpf: ', num2str(kp_bpf)]);
    disp(['error_bpf: ', num2str((kp - kp_bpf)/kp * 100)]);
    ...
    res=kp1;
    disp(['kp_AKM (res): ', num2str(res)]);
    disp(['error_AKM: ', num2str((kp - res)/kp * 100)]);
    disp('---');
end
pause;
